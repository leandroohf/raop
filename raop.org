...
*** DONE Logistic Regredssion
    ...
    #+begin_src R
      ...
      set.seed(2014)
      tic("Training: GLM .....")
      loit_m <- train(requester_received_pizza ~
...
*** Ensemble
   ...
   #+begin_src R
      ...
      train_te$logit_prob <-  predict(logit_m, train_te[, ind_vars], type = 'raw')
      train_te$rf_prob    <-  predict(rf_m, train_te[, ind_vars], type = 'raw')
      train_te$gbm_prob   <-  predict(gbm_m, train_te[, ind_vars],type ='raw')
      train_te$nnet_prob  <-  predict(nnet_m, train_te[, ind_vars], type = 'raw')
      train_te$svm_prob   <-  predict(svm_m, train_te[, ind_vars], type = 'raw')

      train_es$logit_prob <- predict(logit_m, train_es[, ind_vars], type = 'raw')
      train_es$rf_prob    <- predict(rf_m, train_es[, ind_vars], type = 'raw')
      train_es$gbm_prob   <- predict(gbm_m, train_es[, ind_vars],type ='raw')
      train_es$nnet_prob  <- predict(nnet_m, train_es[, ind_vars], type = 'raw')
      train_es$svm_prob   <- predict(svm_m, train_es[, ind_vars], type = 'raw')
      ...
      predictors <- names(train_es)[names(train_es) != labelName]


      stack_m  <- train(x = train_es[, predictors], y = train_es$requester_received_pizza,
                        method = 'gbm', tuneGrid = gbm_tune,
                        metric = 'ROC', verbose = F, trControl = ctrl)
...
*** Comparing Models
   ...
   #+begin_src R
      ...
      random_p <- data.frame(fail = rep(0.5, nrow(train_te)),
                             success = rep(0.5, nrow(train_te)))
      ...
      roc(train_te$requester_received_pizza, random_p$success)
      roc(train_te$requester_received_pizza, null_p$success)
